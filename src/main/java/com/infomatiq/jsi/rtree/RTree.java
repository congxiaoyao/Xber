...
//            new SortedQueue(SortedQueue.SORT_ORDER_ASCENDING);
     ...
     */
    private Node splitNode(Node n, float newRectMinX, float newRectMinY, float newRectMaxX,
                           float newRectMaxY, int newId) {
...
//    }
        ...
        if (INTERNAL_CONSISTENCY_CHECKING) {
            ...
            if (!nMBR.equals(calculateMBR(n))) {
                throw new RuntimeException("Error: splitNode old node MBR wrong");
            }
            ...
            if (!newNodeMBR.equals(calculateMBR(newNode))) {
                throw new RuntimeException("Error: splitNode new node MBR wrong");
            }
        }
...
//    }
     ...
     */
    private void pickSeeds(Node n, float newRectMinX, float newRectMinY, float newRectMaxX,
                           float newRectMaxY, int newId, Node newNode) {
...
//    }
        ...
        for (int i = 0; i < n.entryCount; i++) {
            ...
            if (normalizedSeparation > 1 || normalizedSeparation < -1) {
                throw new RuntimeException("Invalid normalized separation X");
            }
...
//      }
        ...
        for (int i = 0; i < n.entryCount; i++) {
            ...
            if (normalizedSeparation > 1 || normalizedSeparation < -1) {
                throw new RuntimeException("Invalid normalized separation Y");
            }
...
//    }
        ...
        for (int i = 0; i < maxNodeEntries; i++) {
            if (entryStatus[i] == ENTRY_STATUS_UNASSIGNED) {
                ...
                if (n.ids[i] == -1) {
                    throw new RuntimeException("Error: Node " + n.nodeId + ", entry " + i + " is null");
                }
...
//        }
    ...
    private Node chooseNode(float minX, float minY, float maxX, float maxY, int level) {
        ...
        while (true) {
            if (n == null) {
                throw new RuntimeException("Could not get root node (" + rootNodeId + ")");
            }
    ...
    private Node adjustTree(Node n, Node nn) {
        ...
        while (n.level != treeHeight) {
            ...
            if (parent.ids[entry] != n.nodeId) {
                throw new RuntimeException("Error: entry " + entry + " in node " +
    ...
    private boolean checkConsistency(int nodeId, int expectedLevel, Rectangle expectedMBR) {
        ...
        if (!actualMBR.equals(calculatedMBR)) {
            if (actualMBR.minX != n.mbrMinX)
                throw new RuntimeException("  actualMinX=" + actualMBR.minX + ", calc=" + calculatedMBR.minX);
            if (actualMBR.minY != n.mbrMinY)
                throw new RuntimeException("  actualMinY=" + actualMBR.minY + ", calc=" + calculatedMBR.minY);
            if (actualMBR.maxX != n.mbrMaxX)
                throw new RuntimeException("  actualMaxX=" + actualMBR.maxX + ", calc=" + calculatedMBR.maxX);
            if (actualMBR.maxY != n.mbrMaxY)
                throw new RuntimeException("  actualMaxY=" + actualMBR.maxY + ", calc=" + calculatedMBR.maxY);
            return false;
        }
